#!/usr/bin/env python3
"""
Testing/test_cluster1_risk_narrative.py

Runs ONE risk_narrative generation using the same prompt + generation path as cluster1_cli.py.
"""
...
SAMPLE_RISK_NOTES = """We observed suspicious logins to an internal admin console. Scope is unclear. No confirmed exfiltration. ..."""


def read_optional_text(text: str | None, text_file: str | None) -> str:
    if text is None and text_file is None:
        return SAMPLE_RISK_NOTES
...
def main() -> None:
    ...
    ap.add_argument("--lora-dir", default=DEFAULT_LORA_RISK)
    ...
    messages = build_messages("risk_narrative", style, raw)
    out1 = generate(
        model=model,
        tokenizer=tokenizer,
        messages=messages,
    ...
    if audit and style == "strict":
        audit_msgs = build_audit_messages("risk_narrative", raw, out1)
        out2 = generate(
            model=model,
            tokenizer=tokenizer,
            messages=audit_msgs,
