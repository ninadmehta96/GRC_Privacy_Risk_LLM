...
Version-tolerant QLoRA SFT trainer for Cluster-1 policy_refactor adapter.

Dataset format (JSONL):
  {"id": "...", "split": "train|dev|test", "messages": [{"role":"...", "content":"..."}]}

This script maps messages -> single 'text' field via tokenizer.apply_chat_template().
Because we use a plain text dataset, assistant_only_loss MUST be False (TRL requires
conversational datasets for assistant_only_loss=True).
"""
...
DEFAULT_DATA_PATH = str(
    _pick_existing(
        [
            REPO_ROOT / "data" / "training_data" / "v2" / "grc_policy_refactor_v2_strict.jsonl",
            REPO_ROOT / "data" / "training_data" / "grc_policy_refactor_v2_strict.jsonl",
            REPO_ROOT / "data" / "training_data" / "v1" / "grc_policy_refactor_v1_1.jsonl",
            REPO_ROOT / "data" / "training_data" / "grc_policy_refactor_v1_1.jsonl",
        ]
    )
)

DEFAULT_OUTPUT_DIR = str(REPO_ROOT / "adapters" / "mistral7b-cluster1-policy-refactor-lora-v2-strict")
...
def _build_sft_config_compat(**kwargs) -> SFTConfig:
    ...
    - filter unknown keys based on signature
    """
...
def _build_trainer_compat(**kwargs) -> SFTTrainer:
    """
    TRL compatibility:
    - some versions use processing_class=tokenizer, others use tokenizer=tokenizer
    - filter unknown keys based on signature
    """
...
def main() -> None:
    ...
    sft_args = _build_sft_config_compat(
        ...
        report_to="none",
        evaluation_strategy=eval_strategy_value,   # auto-mapped if needed
    ...
    print("Saved policy_refactor LoRA to:", args.output_dir)
